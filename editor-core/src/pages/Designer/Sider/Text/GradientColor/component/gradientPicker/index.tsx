...
const GradientPicker = (props: any) => {
  const {
    current,
    list,
    background,
  ...
  const { run: runChangeList } = useThrottleFn(
    value => {
      // value.sort((item, nextItem) => {
      //   return item.percent < nextItem.percent;
      // });
      changeList(value);
    },
    { wait: 60 },
  ...
  const { run: calculate } = useThrottleFn(
    event => {
      ...
      const exist =
        icmList.filter(pct => {
          return pct.percent === item.percent;
        }).length > 0;
      // 当前比例存在
      if (exist) return;
