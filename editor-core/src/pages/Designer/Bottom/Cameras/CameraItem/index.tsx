...
import ClipItem from '@/components/TimeLine/components/ClipItem/ClipItem';
...
const CameraItem = (props: {
  ...
  // 镜头的最短时间限制宽度
  const cameraMinWidth = calcTimeToPx(CAMERA_MIN_DURATION, unitWidth);

  const width = calcTimeToPx(endTime - startTime, unitWidth);
  const left = calcTimeToPx(startTime, unitWidth);
  ...
  const handleMouseDown = (changeType: ChangeType, e: MouseEvent) => {
    ...
    limit.current = {
      min: 0,
      max: template.videoInfo.allAnimationTime,
    };
  ...
  useMouseHandle({
    ...
    onMouseMove: info => {
      if (originCamera.current && limit.current) {
        let newStart =
          originCamera.current.startTime +
          calcPxToTime(info.distanceX, unitWidth);
  ...
  const calcLineWidth = () => {
    ...
    if (index === cameras.length - 1) {
      endT = template.videoInfo.allAnimationTime;
    } else {
      endT = cameras[index + 1].camera.startTime;
    }
    const widthT = calcTimeToPx(endT - startT, unitWidth);
    return widthT;
  };
