...
const useNewPsd = () => {
    ...
    if (canvas.current) {
      ...
      textList.current.forEach((item: any) => {
        ...
        if (item.node.getScaledHeight()) {
          height = Math.max(item.node.getScaledHeight() * item.data.scale, height);
        }
  ...
  function formatStrokesData(fdata: any) {
    ...
    supportTexts.forEach((item: any) => {
      ...
      if (dataLep.strokes.length > 1) {
        ...
        let genDiff = 0;
        let strokeWidth = 0;
        if (strokeList.length > 0 && dataLep.strokes.length > 0 && item.strokes.length > 0) {
  ...
  function changeNewPsdFontAttr(fontParam: any) {
    ...
    if (font) {
      ...
      //  @ts-ignore
      fontParam.charSpacing = (fontParam.letterSpacing * 1000) / fontParam.fontSize;
      ...
      textList.current.forEach((item: any) => {
        ...
        if (item.node.getScaledHeight()) {
          height = Math.max(item.node.getScaledHeight() * fontParam.scale, height)
        }
      });
      setCanvasAttr(fontParam.width * fontParam.scale + maxStrokeWidth, height + maxStrokeWidth);
      // canvas.current.renderAll();
