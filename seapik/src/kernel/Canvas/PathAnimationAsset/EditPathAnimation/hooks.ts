...
const usePathAnimationStyle = (asset: Asset, canvasInfo: CanvasInfo) => {
    const getCenterPoint = () => {
        if (asset) {
            ...
            // 参考点位置
            const { x: originLeft, y: originTop } = getCenterPointFromSize({ left, top }, { width, height })
            return {
                left: originLeft,
                top: originTop
            }
    ...
    const holderStyle = useMemo(() => {
        return { width: canvasInfo.width, height: canvasInfo.height, background: 'pink', position: 'absolute', display: 'none' }
    }, [asset?.meta])
    return {
        getCenterPoint,
        holderStyle
