...
const Lllegal = (props: AssetItemProps) => {
  ...
  const isLllegel = lllegelAssets.filter((item) => {
    if (isMaskType(asset) && asset.assets.length > 0) {
      return (
        (item.resId === attribute.resId ||
          asset.assets[0].attribute.resId === item.resId) &&
        item.scan_flag === 3
      );
    } else {
  ...
  const BackgroundSrc =
    isLllegel.length > 0
      ? 'https://js.xiudodo.com/xiudodo-editor/image/error/lllegelIcon.png'
      : 'https://js.xiudodo.com/xiudodo-editor/image/error/sourceLoss.png';

  return (
    <>
      <div
        className={styles.lllegal}
        style={{
